...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # if noiseDistance is none ( pacman is captures) then there is only one element in self.beliefs and its jailPosition with probability equal to 1
        ...
        # if noiseDistance is not none then we should get new sel.beliefs table
        # we need to multiply on P(noisyDistance | TrueDistance) for every position and then normalize
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # when time elapses P(X(t+1)|e(1..t)) = sum [ (P(X(t+1)) * P(x(t) | e(1...t))) ]
        # old prob is what was probability of being in some possition
        # prob is what is probability of getting in newPos from oldPos ,
        # so I sum up products of this probabilities to get final probability of being in newPos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        # just create same amount of particles for each position in legalPositions
        self.particles = list()
    ...
    def observe(self, observation, gameState):
        ...
        # if noisiDistance in none all particles should be prison cell , as stated in problem statement
        if noisyDistance is None:
            self.particles = []
        ...
        # gives each sample its weight based on new observe
        # stores them in util counter , and then normalize
        allPossible = util.Counter()
        ...
        # if weights of all particles are zero we should call initial normalizy uniformly again
        # else should choose new particles randomly based on possible positions weights
    ...
    def elapseTime(self, gameState):
        ...
        a belief distribution.
        """
        # get new particles from old particles as time elapses
        # for each sample get all possible actions with its probabilities and choose randomly based on those probabilities
        newParticle = []
        for sample in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, sample))
    ...
    def getBeliefDistribution(self):
        ...
        Counter object)
        """
        # count quantity of each position which is in particles, and then normalize 
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # in legal there is all possible combination of positions where ghosts can be
        # choose equally from it every combination
        ...
        for index in range(0 , len(self.particles)):
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    self.particles[index] = self.getParticleWithGhostInJail(sample, i)
                    sample = self.particles[index]
                else:
                    trueDistance = util.manhattanDistance(sample[i], pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # for each ghost choose next postion randomly
    ...
    def getBeliefDistribution(self):
        # count how many times do we have each combination of ghosts and the normilize
        beliefs = util.Counter()
        for s in self.particles:
            beliefs[s] += 1
        beliefs.normalize()
