...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # count how many regions does the picture has and add it as features
    regions = countRegions(datum)
    features["one_regions"] = 1 if regions == 1 else 0
    features["two_regions"] = 1 if regions == 2 else 0
    features["more_than_two_regions"] = 1 if regions >= 3 else 0
    return features
    

# just simple dfs, calls dfs for all its neighbours and adds itself in been
been = set()
def dfs(datum , h , w):
    if(h < 0 or h >= DIGIT_DATUM_HEIGHT): return
    if(w < 0 or w >= DIGIT_DATUM_HEIGHT): return
    if (h , w) in been: return
    been.add((h , w))
    if datum.getPixel(h , w) >= 1: return
    dfs(datum , h + 1 , w)
    dfs(datum , h - 1 , w)
    dfs(datum , h , w + 1)
    dfs(datum , h , w - 1)

# calls dfs for every point where has not already been and is black
# number of calls will be result
def countRegions(datum):
    been.clear()
    res = 0
    for h in range(DIGIT_DATUM_HEIGHT):
        for w in range(DIGIT_DATUM_WIDTH):
            if (h , w) in been: continue
            if datum.getPixel(h , w) >= 1: continue
            dfs(datum , h , w)
            res += 1
    return res
...
def enhancedPacmanFeatures(state, action):
    ...
    foods = newGameState.getFood().asList()
    ghosts = newGameState.getGhostStates()
    position = newGameState.getPacmanPosition()
    nearest_food = sys.maxsize
    nearest_ghost = sys.maxsize
   
    # count manhattan distance to nearest food
    for food in foods:
        nearest_food = min(nearest_food ,util.manhattanDistance(position , food))
    
    # count manhattan distance to nearest ghots
    for ghost in ghosts:
        nearest_ghost = min(nearest_ghost ,util.manhattanDistance(position , ghost.getPosition()))

    # nearest ghost distance feature
    features["ghost"] = nearest_ghost

    # nearest food distance feature
    features["food"] = 2.0 / nearest_food 
